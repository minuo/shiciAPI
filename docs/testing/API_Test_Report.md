...
| 端点 | 方法 | 状态 | 响应时间 | 备注 |
|------|------|------|----------|------|
| 获取作者列表 | GET /api/authors | 通过 | - | 支持分页、朝代筛选、关键词搜索 |
| 获取作者列表-带分页参数 | GET /api/authors?page=1&size=10 | 通过 | - | - |
| 获取作者列表-按朝代筛选 | GET /api/authors?dynasty=宋代 | 通过 | - | - |
| 获取作者详情 | GET /api/authors/:id | 通过 | - | - |
| 获取作者诗词列表 | GET /api/authors/:id/poems | 通过 | - | 支持分页和排序 |
| 创建作者-权限检查 | POST /api/authors | 通过 | - | 需要管理员权限 |
| 更新作者-权限检查 | PUT /api/authors/:id | 通过 | - | 需要管理员权限 |
| 删除作者-权限检查 | DELETE /api/authors/:id | 通过 | - | 需要管理员权限
